...
function GeneTrack(gsvg,data,trackClass,label,additionalOptions){
	...
	that.setupDetailedView=function(d){
		if(d){
			...
			if(that.trackClass.indexOf("liverTotal")===0){
			...
			var newMin=0;
			var newMax=0;
			if(localTxType==="protein"||localTxType==="long"||localTxType.indexOf("liverTotal")===0||localTxType==="heartTotal"||localTxType.indexOf("brainTotal")===0||localTxType==="mergedTotal"|| (localTxType==="small" && genomeVer!=="rn5") ||(localTxType==="small"&& new String(d.getAttribute("ID")).indexOf("ENS")>-1)){
	...
	that.updateData=function(retry){
		...
		d3.xml(path,function (error,d){
			if(error){
				if(retry===0  && (that.trackClass.indexOf("liverTotal")===0 || that.trackClass.indexOf("brainTotal")===0 || that.trackClass==='mergedTotal' || that.trackClass.indexOf("smallnc")>-1)){
                	var tmpContext=contextPath +"/"+ pathPrefix;
					if(!pathPrefix){
						tmpContext="";
					}
		...
		for(var m=0;m<exList.length;m++){
			...
			if((exStrt<cdsStart&&cdsStart<exStp)||(exStp>cdsStop&&cdsStop>exStrt)){//need to draw two rect one for CDS and one non CDS
				...
				txG.append("rect")
			    	...
			    	.attr("height",height)
					.attr("width",function(d){ var tmp= that.xScale(exList[m].getAttribute("stop")) - that.xScale(exList[m].getAttribute("start")); return tmp;})
					.attr("title",function(d){ return exList[m].getAttribute("ID");})
					.style("fill",that.color)
					.style("cursor", "pointer");
			}
	...
	that.draw=function (data){
		...
		}else if(that.trackClass.indexOf("brainTotal")===-1&&that.trackClass.indexOf("liverTotal")===-1&&that.trackClass!="heartTotal"&&that.trackClass!="mergedTotal"){
			...
			if(that.drawAs==="Gene" || that.trackClass.indexOf("smallnc")>-1){
					...
					.on("mouseover", function(d) {
							...
							if(that.gsvg.isToolTip===0){
					            ...
					            if(d){
									...
									tt.style("top", function(){return that.positionTTTop(d3.event.pageY);});
								}
							}
						//}
	                                        //return false;
				...
				var tx=that.svg.selectAll(".trx"+that.gsvg.levelNumber)
			  	...
			  	tx.enter().append("g")
						...
						.attr("transform",function(d,i){ return "translate(0,"+that.calcY(parseInt(d.getAttribute("start"),10),parseInt(d.getAttribute("stop"),10),i)+")";})
						.attr("id",function(d){
							var prefix="";
							if(that.gsvg.levelNumber==1){
								prefix="tx";
	...
	that.draw= function (data){
		...
		if(that.colorSelect=="dabg"||that.colorSelect=="herit"){
			...
			for(var t=0;t<that.tissues.length;t++){
						...
						if(data.length>0){
							var probes=that.svg.selectAll(".probe."+tissue)
								  ...
								  .attr("transform",function(d,i){ return "translate("+that.xScale(d.getAttribute("start"))+","+(that.calcY(parseInt(d.getAttribute("start"),10),parseInt(d.getAttribute("stop"),10),i,d.getAttribute("ID").length)+totalYMax*15-10)+")";});
							//add new
			...
			var probes=that.svg.selectAll(".probe.annot")
				...
				.attr("transform",function(d,i){ return "translate("+that.xScale(d.getAttribute("start"))+","+that.calcY(parseInt(d.getAttribute("start"),10),parseInt(d.getAttribute("stop"),10),i,d.getAttribute("ID").length)+")";});
							...
							if(retry==0||force==1){
								...
								$.ajax({
													url: tmpContext +"generateTrackXML.jsp",
									   				type: 'GET',
									   				cache: false,
													data: {chromosome: chr,minCoord:tmpMin,maxCoord:tmpMax, myOrganism: organism,genomeVer:genomeVer, track: that.trackClass,bedFile: bedFile,outFile:file, folder: that.gsvg.folderName,binSize:that.bin,type:type,web:web},
	...
	that.redraw = function (){
		if(that.prevDensity!=that.density){
			...
			var txG=d3.select("#Level"+that.gsvg.levelNumber+that.trackClass)
				...
				.attr("transform",function(d,i){ return "translate("+that.xScale(d.getAttribute("start"))+","+that.calcY(parseInt(d.getAttribute("start"),10),parseInt(d.getAttribute("stop"),10),i,d)+")";});
						...
						if(retry==0 || force==1){
							...
							$.ajax({
								url: tmpContext +"generateTrackXML.jsp",
				   				type: 'GET',
				   				cache: false,
								data: {chromosome: chr,minCoord:tmpMin,maxCoord:tmpMax,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID: arrayTypeID, myOrganism: organism,genomeVer:genomeVer, track: that.trackClass, folder: that.gsvg.folderName},
								//data: {chromosome: chr,minCoord:minCoord,maxCoord:maxCoord,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID: arrayTypeID, myOrganism: organism, track: that.trackClass, folder: folderName,binSize:that.bin},
								dataType: 'json',
				    			success: function(data2){
				    				if(ga){
