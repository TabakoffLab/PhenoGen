...
var siteVer="PhenoGen v2.15.4(5/22/2015)";
...
function loadSavedConfigTracks(trackListObj,levelInd,curSvg){
	...
	console.log("loadSavedConfigTracks");
...
function GenomeSVG(div,imageWidth,minCoord,maxCoord,levelNumber,title,type){
	...
	that.addTrack=function (track,density,additionalOptions,retry){
		console.log("Genome Add Track:"+track);
				...
				d3.xml(file,function (error,d){
					if(error){
						...
						if(d==null){
							if(retry>=11){
								var data=new Array();
								var newTrack= RepeatMaskTrack(that,data,track,"Repeat Masker",additionalOptions);
								that.addTrackList(newTrack);
							}else{
								setTimeout(function (){
									that.addTrack(track,density,additionalOptions,11);
	...
	that.setLoading=function (){
		for(var i=0;i<that.trackList.length;i++){
			if(typeof that.trackList[i]!=='undefined' && 
				(typeof that.trackList[i].updateData!=='undefined' || typeof that.trackList[i].updateFullData!=='undefined')){
...
function toolTipSVG(div,imageWidth,minCoord,maxCoord,levelNumber,title,type){
	...
	that.addTrack=function (track,density,additionalOptions,data){
		console.log("toolTip addTrack("+track+","+density+","+additionalOptions+"  "+data+")");
		if(that.forceDrawAsValue=="Trx"){
			var additionalOptionsStr=new String(additionalOptions);
			if(additionalOptionsStr.indexOf("DrawTrx")==-1){
				additionalOptions=additionalOptions+"DrawTrx,";
...
function GeneTrack(gsvg,data,trackClass,label,additionalOptions){
	...
	that.setSelected=function(geneID){
		if(geneID){
			if(d3.selectAll("g.gene").size()>0){
				...
				var tmp;
				if(that.svg.selectAll("g.gene"+str)[0][0]){
	...
	that.getDisplayedData= function (){
		...
		if(that.drawnAs==="Gene"){
			...
			var trxList=d3.select("#Level"+that.gsvg.levelNumber+that.trackClass).selectAll("g.trx"+that.gsvg.levelNumber)[0];
			that.counts=new Array();
			var countsInd=0;
			var total=0;
			if(typeof trxList !=='undefined'){
				for(var l=0;l<trxList.length;l++){
					var start=that.xScale(trxList[l].__data__.getAttribute("start"));
					var stop=that.xScale(trxList[l].__data__.getAttribute("stop"));
					if((0<=start && start<=that.gsvg.width)||(0<=stop &&stop<=that.gsvg.width)||(start<=0&&stop>=that.gsvg.width)){
						...
						var name="Ensembl";
						if(typeof trxList[l].__data__.getAttribute("strain") !=='undefined'){
							name=trxList[l].__data__.getAttribute("strain");
						}
						...
						dispData[dispDataCount]=trxList[l].__data__;
						dispDataCount++;
						total++;
					}
				}
	...
	that.setupToolTipSVG=function(d,perc){
		...
		var tmpMin=0;
        var tmpMax=0;
        if(typeof d.getAttribute("extStart")!=='undefined'){
	...
	that.createToolTip=function(d){
		...
		if(that.drawnAs==="Gene"){
			...
			tooltip="<BR><div id=\"ttSVG\" style=\"background:#FFFFFF;\"></div>Gene Symbol: "+d.getAttribute("geneSymbol")+"<BR>Location: "+d.getAttribute("chromosome")+":"+numberWithCommas(d.getAttribute("start"))+"-"+numberWithCommas(d.getAttribute("stop"))+"<BR>Strand: "+d.getAttribute("strand")+"<BR>Transcripts:<BR>"+txListStr;
				...
				txListStr="<B>"+id+"</B>";
			tooltip="<BR><div id=\"ttSVG\" style=\"background:#FFFFFF;\"></div>RefSeq ID:"+txListStr+"<BR>Status: "+d.getAttribute("category")+"<BR>Gene Symbol: "+d.getAttribute("geneSymbol")+"<BR>Location: "+d.getAttribute("chromosome")+":"+numberWithCommas(d.getAttribute("start"))+"-"+numberWithCommas(d.getAttribute("stop"))+"<BR>Strand: "+d.getAttribute("strand");
		}
		return tooltip;
	};
		...
		if(that.curColor!=that.colorSelect || ((that.colorSelect==="herit" || that.colorSelect==="dabg") && tissueLen!=that.tissueLen)){
			...
			if(that.colorSelect=="dabg"||that.colorSelect=="herit"){
					...
					that.trackYMax=0;
						...
						that.svg.selectAll("g.probe."+tissue)
									.attr("transform",function (d,i){
									   	...
									   	var y=that.calcY(d.getAttribute("start"),d.getAttribute("stop"),i,d.getAttribute("ID").length)+totalYMax*15-10;
				...
				that.svg.selectAll("g.probe")
					.attr("transform",function (d,i){
						var st=that.xScale(d.getAttribute("start"));
						return "translate("+st+","+that.calcY(d.getAttribute("start"),d.getAttribute("stop"),i,d.getAttribute("ID").length)+")";
		...
		if(that.colorSelect=="dabg"||that.colorSelect=="herit"){
			...
			for(var t=0;t<that.tissues.length;t++){
						...
						var probes=that.svg.selectAll(".probe."+tissue)
							...
							.attr("transform",function(d,i){ return "translate("+that.xScale(d.getAttribute("start"))+","+(that.calcY(d.getAttribute("start"),d.getAttribute("stop"),i,d.getAttribute("ID").length)+totalYMax*15-10)+")";});			
						//add new
			...
			var probes=that.svg.selectAll(".probe.annot")
				...
				.attr("transform",function(d,i){ return "translate("+that.xScale(d.getAttribute("start"))+","+that.calcY(d.getAttribute("start"),d.getAttribute("stop"),i,d.getAttribute("ID").length)+")";});
...
function SNPTrack(gsvg,data,trackClass,density,additionalOptions){
	...
	that.getDisplayedData= function (){
		...
		if(typeof tmpDat!=='undefined'){
		for(var l=0;l<tmpDat.length;l++){
			if(typeof tmpDat[l].__data__!=='undefined'){
				var start=that.xScale(tmpDat[l].__data__.getAttribute("start"));
				var stop=that.xScale(tmpDat[l].__data__.getAttribute("stop"));
				if((0<=start && start<=that.gsvg.width)||(0<=stop &&stop<=that.gsvg.width)){
					var ind=0;
...
function RepeatMaskTrack(gsvg,data,trackClass,label,density,additionalOptions){
	...
	that.drawTrx=function (d,i){
		...
		if(exList.length===1){
			...
			.attr("x",function(d){ return that.xScale(exList[0].getAttribute("start"))-that.xScale(d.getAttribute("start")); })
			.attr("rx",1)
			.attr("ry",1)
	    	.attr("height",10)
			.attr("width",function(d){ 
	...
	that.redrawLegend=function (){
		var legend=[];
		legend[0]={color:"#FFFFFF",label:"SINE"};
		legend[1]={color:"#FFFFFF",label:"LINE"};
		legend[2]={color:"#FFFFFF",label:"LTR"};
		legend[3]={color:"#FFFFFF",label:"DNA"};
		legend[4]={color:"#FFFFFF",label:"Simple"};
		legend[5]={color:"#FFFFFF",label:"Low Comp."};
		legend[6]={color:"#FFFFFF",label:"Sat."};
		legend[7]={color:"#FFFFFF",label:"RNA"};
		legend[8]={color:"#FFFFFF",label:"Other"};
		legend[9]={color:"#FFFFFF",label:"Unkwn"};
		that.drawLegend(legend);
	};
...
function GenericTranscriptTrack(gsvg,data,trackClass,label,density,additionalOptions){
	...
	that.updateFullData = function(retry,force){
		console.log("calling updateFullData()"+that.dataFileName);
		var tmpMin=that.xScale.domain()[0];
		var tmpMax=that.xScale.domain()[1];
