...
function WGCNABrowser(id,region,geneList,disptype,viewtype,tissue){
	...
	that.createDataControls=function(){
		...
		for(var i=0;i<that.tissues.length;i++){
			sel.append("option").attr("value",that.tissues[i]).text(that.dispTissues[i]);
                ...
                mir.append("text").text(" miRNAs with Validated Targets");
                mir.append("br");
                ...
                mir.append("br");
                mir.append("text").text("Hovering over transcript:");
                mir.append("br");
		...
		mir.append("text").text("Display links to miRNAs trageting the transcript");
                mir.append("br");
                mir.append("input").attr("type","checkbox")
                        .style("margin-left","12px")
                        .style("margin-right","3px")
	...
	that.singleWGCNAImageGeneView=function(){
		var thatimg=that.singleWGCNAImage();
                ...
                thatimg.mirTargetID="";
                thatimg.mirID="";
                thatimg.miRcolor = d3.scale.category20b();
                thatimg.sortMir=0;
			...
			//add new
                ...
                thatimg.drawMiR=function(){
                    if(typeof thatimg.topG!=='undefined' && typeof thatimg.mirData!=='undefined'){
                        ...
                        thatimg.miRbubble = d3.layout.pack()
                                            .sort(thatimg.sortMirFunction)
                                            .size([thatimg.miRdiameter, thatimg.miRdiameter])
                                            .padding(1.5);
                        //console.log(thatimg.svg.select(".svg-pan-zoom_viewport")[0][0]);
                        //var topG=thatimg.svg.select(".svg-pan-zoom_viewport")[0][0];
                        var tmpData=thatimg.runMirFilter(thatimg.mirData.MIRList);
                        var root={ ID:"miRNAs Tageting Genes in Module",name:"miRNAs Tageting Genes in Module",children:tmpData};
                        ...
                        var node = mirG.selectAll(".node")
                                        .data(thatimg.miRbubble.nodes(root),that.mirKey)
                                        .enter().append("g")
                                        .attr("class", "node")
                                        .attr("id", function(d){return replaceDot(d.ID);})
                ...
                thatimg.runMirFilter=function(list){
                    var newList=list;
                    if( thatimg.mirGeneCutoff>1 || thatimg.mirValid===1 || thatimg.mirID.length>0 || thatimg.mirTargetID.length>0){
                        var mc=0;
                        newList=[];
                ...
                thatimg.createMirToolTip=function(d){
                    var txt="";
                    txt="<div id=\"ttSVG\"></div>ID: "+d.ID+"<BR>Accession: "+d.ACC+"<BR>Genes Targeted in Module: "+d.GeneList.length+"<BR>";
                    return txt;
                };
                
                thatimg.mouseoverMir=function(){
                    var tmpD=d3.select(this).data()[0];
                    var genes=tmpD.GeneList;
                    console.log(genes);
                    d3.select(this).style("opacity","0.3");
                    tt.transition()        
                            .duration(200)      
                            .style("opacity", 1);
                    tt.html(thatimg.createMirToolTip(tmpD))
                            .style("left", function(){return that.positionTTLeft(d3.event.pageX);})     
                            .style("top", function(){return that.positionTTTop(d3.event.pageY);});
                    ...
                    lnk.enter().append("line")
                                ...
                                .attr("x1",function(d){
                                    var x=tmpD.x+thatimg.width/2-thatimg.miRdiameter/2;
                                    return x;
                                })
                                .attr("y1",function(d){
                                    var y=tmpD.y+thatimg.height/2-thatimg.miRdiameter/2;
                                    return y;
                                })
                                .attr("x2",function(d){
                                    var x=d3.select("."+d.ID).attr("cx");
                                    //console.log(d.TC2+":x2:"+x);
                                    return x;
                                })
                                .attr("y2",function(d){
                                    var y=d3.select("."+d.ID).attr("cy");
                                    //console.log(d.TC2+":y2:"+y);
                                    return y;
                                })
                ...
                thatimg.mouseoverGene=function(){
                    ...
                    if(that.viewType==='mir'  && thatimg.showMirLinks){
                        ...
                        lnk.enter().append("line")
                                ...
                                .attr("id",function(d){return "ln_"+d.ID;})
                                .attr("x1",function(d){
                                    var x=d3.select("#"+replaceDot(d.ID))[0][0].__data__.x+thatimg.width/2-thatimg.miRdiameter/2;
                                    return x;
                                })
                                .attr("y1",function(d){
                                    var y=d3.select("#"+replaceDot(d.ID))[0][0].__data__.y+thatimg.height/2-thatimg.miRdiameter/2;
                                    return y;
                                })
                                .attr("x2",function(d){
	...
	that.singleWGCNATableGeneView=function(){
        ...
        that.singleWGCNATableMirView=function(){
            ...
            thattbl.createMirTable = function(){
                    ...
                    var tracktbl=d3.select("table#mirTable").select("tbody").selectAll('tr')
                            .data(that.singleImage.mirData.MIRList,that.mirKey)
                                    .enter().append("tr")
                                    .attr("id",function(d){return "mirTR"+d.ID;})
                                    .attr("class",function(d,i){var ret="odd";if(i%2===0){ret="even";} return ret;});
                    tracktbl.each(function(d,i){
                                    var total=d.GeneList.length;
                                    var pred=total-d.vC;
                                    d3.select(this).append("td").html(d.ID);
                                    d3.select(this).append("td").html(d.ACC);
                                    d3.select(this).append("td").html("");
                                    d3.select(this).append("td").html(pred);
                                    d3.select(this).append("td").html(d.vC);
                                    d3.select(this).append("td").html(total);
                    });
		...
		tracktbl.each(function(d,i){
				...
				d3.select(this).append("td").html("");
				d3.select(this).append("td").html(pred);
                                d3.select(this).append("td").html(val);
                                d3.select(this).append("td").html(function(){var len=0; if(typeof miList !=='undefined'){len=miList.length;} return len;});
		});
	...
	that.singleWGCNAImageGoView=function(){
            ...
            thatimg.redraw=function(){
                ...
                if(thatimg.selectedNode!==thatimg.data.GOList[0]||thatimg.selectedNode!==thatimg.data.GOList[1]||thatimg.selectedNode!==thatimg.data.GOList[2]){
                    thatimg.svg.append("circle").attr("id","center").attr("cx",Math.max(thatimg.width/2,thatimg.radius) + thatimg.padding).attr("cy",Math.max(thatimg.height/2,thatimg.radius) + thatimg.padding).attr("r","20")
                        ...
                        .attr("pointer-events","all")
                        .on("click",function(){
                            if(typeof thatimg.selectedNode.parent !=='undefined' ){
            ...
            thatimg.draw=function(){
                ...
                d3.json(contextRoot+"tmpData/modules/ds"+that.wDSID+"/"+replaceUnderscore(that.selectedModule.MOD_NAME)+".GO.json", function(error, root) {
                    ...
                    thatimg.svg.append("g")
                            ...
                            .text(goRoot.uniqueGene+" genes in module with GO annoation");    
        ...
        that.singleWGCNATableGoView=function(){
            ...
            /*var root=that.singleImage.data.GOList[0];
            if(that.singleImage.goDomain === "mf"){
                root=that.singleImage.data.GOList[2];
            }else if(that.singleImage.goDomain === "bp"){
                root=that.singleImage.data.GOList[1];
            }*/
            ...
            $("#GoTable .trigger").on("click",function(){
            ...
            $("#GoTable .triggerGL").on("click",function(){
