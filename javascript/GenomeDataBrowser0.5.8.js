...
function SequenceTrack(gsvg,trackClass,label,additionalOptions){
	...
	that.transcribe=function(d){
		d=d.toUpperCase();
		return that.codons[d];
	}
	that.transcribeRegion=function(aaLevel,offset,start,stop){
		var aaSeq=new Array();
		var aaCount=0;
		var minus=0;
		...
		for(var i=start;i<=stop;i=i+3){
				...
				aaSeq[aaCount]=[];
				aaSeq[aaCount].aa=that.transcribe(seq);
				aaSeq[aaCount].pos=offset+(i-start);
				aaSeq[aaCount].id=aaLevel+":"+i+":"+that.transcribe(seq);
				aaCount++;
		}
		return aaSeq;
	}
				...
				if(!(that.seqRegionMin<=tmpMin && tmpMin<=that.seqRegionMax
					...
					if(that.includeAA==1){
						...
						if(that.strands=="both"||that.strands=="+"){
							...
							aaList[0]=that.transcribeRegion(0,tmpMin-modStart,zeroStart,zeroStart+len);
							aaList[1]=that.transcribeRegion(1,tmpMin-modStart-1,zeroStart-1,zeroStart-1+len);
							aaList[2]=that.transcribeRegion(2,tmpMin-modStart-2,zeroStart-2,zeroStart-2+len);
							var tmpLineAt=25;
							for(var j=0;j<3;j++){
						...
						if(that.strands=="both"||that.strands=="-"){
							...
							var zeroStart=startInd-modStart;
							var aaList=new Array();
							aaList[0]=that.transcribeRegion(0,tmpMin-modStart,zeroStart+len,zeroStart);
							aaList[1]=that.transcribeRegion(1,tmpMin-modStart-1,zeroStart-1+len,zeroStart-1);
							aaList[2]=that.transcribeRegion(2,tmpMin-modStart-2,zeroStart-2+len,zeroStart-2);
							for(var j=0;j<3;j++){
		...
		if(that.willDraw==1&&((len<=that.aaDispCutoff&&that.includeAA==1)||(len<=that.dispCutoff))){
			...
			if(that.includeAA==1){
				...
				if(that.strands=="both"||that.strands=="+"){
					...
					var aaList=new Array();
					aaList[0]=that.transcribeRegion(0,tmpMin-modStart,zeroStart,zeroStart+len);
					aaList[1]=that.transcribeRegion(1,tmpMin-modStart-2,zeroStart-2,zeroStart-2+len);
					aaList[2]=that.transcribeRegion(2,tmpMin-modStart-1,zeroStart-1,zeroStart-1+len);
					var tmpLineAt=25;
					for(var j=0;j<3;j++){
						var aa=that.svg.selectAll(".aa"+j)
			   				.data(aaList[j],keyID)
			...
			if(error){
				...
				if(retry==0){
							$.ajax({
								url: contextPath +"/"+ pathPrefix +"generateTrackXML.jsp",
				   				type: 'GET',
								data: {chromosome: chr,minCoord:tmpseqRegionMin,maxCoord:tmpseqRegionMax,panel:panel,rnaDatasetID:rnaDatasetID,arrayTypeID: arrayTypeID, myOrgansim: organism, track: that.trackClass, folder: folderName},
								dataType: 'json',
				    			success: function(data2){
				...
				if(retry<3){//wait before trying again
						...
						setTimeout(function (){
							...
							that.seqRegionMax=0;
							that.hideLoading();
							d3.select("#Level"+this.gsvg.levelNumber+this.trackClass).select("#trkLbl").text("An errror occurred loading Track:"+track);
							d3.select("#Level"+this.gsvg.levelNumber+this.trackClass).attr("height", 15);
				}
			}else{
	...
	that.createToolTip=function(d){
		...
		if((d.getAttribute("stop")-d.getAttribute("start"))<200){
			...
			if(d.parent!=undefined){
				...
				tooltip="Transcript ID: "+gid+"<BR>Gene Symbol:"+geneSym+"<BR>Location:"+d.getAttribute("chromosome")+":"+d.getAttribute("start")+"-"+d.getAttribute("stop")+"<BR>Strand:"+strand;
				tooltip=tooltip+"<BR><BR>Gene ID: "+that.getDisplayID(d.parent.getAttribute("ID"))+"<BR>Other Transcripts:<BR>"+txListStr;
			}else{
				...
				tooltip="Gene ID: "+gid+"<BR>Gene Symbol:"+geneSym+"<BR>Location:"+d.getAttribute("chromosome")+":"+d.getAttribute("start")+"-"+d.getAttribute("stop")+"<BR>Strand:"+strand+"<BR>Transcripts:<BR>"+txListStr;
			}
		  	...
		  	tx.enter().append("g")
					...
					.on("click", function(d){
						if(that.gsvg.levelNumber==0){
							d3.selectAll("g.trx").selectAll("line").style("stroke",that.color);
							d3.selectAll("g.trx").selectAll("rect").style("fill",that.color);
							d3.selectAll("g.trx").selectAll("text").style("opacity","0.6").style("fill",that.color);
							d3.selectAll("g."+d.parent.getAttribute("ID")).selectAll("line").style("stroke","green");
							d3.selectAll("g."+d.parent.getAttribute("ID")).selectAll("rect").style("fill","green");
							d3.selectAll("g."+d.parent.getAttribute("ID")).selectAll("text").style("opacity","0.3").style("fill","green");
					...
					.on("mouseover", function(d) { 
							if(that.gsvg.levelNumber==0&&d.parent!=undefined){
								...
								d3.selectAll("g."+str).selectAll("line").style("stroke","green");
								d3.selectAll("g."+str).selectAll("rect").style("fill","green");
								d3.selectAll("g."+str).selectAll("text").style("opacity","0.3").style("fill","green");
							}else{
					...
					.on("mouseout", function(d) {
						if(that.gsvg.levelNumber==0){
							var str=(new String(d.parent.getAttribute("ID"))).replace(".","_");
							d3.selectAll("g."+str).selectAll("line").style("stroke",that.color);
							d3.selectAll("g."+str).selectAll("rect").style("fill",that.color);
							d3.selectAll("g."+str).selectAll("text").style("opacity","0.6").style("fill",that.color);
						}else{
