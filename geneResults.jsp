...
This tab has heat maps for the expression and fold difference between the Parental Strians(Rat BN-Lx/SHR or Mouse ILS/ISS).  To switch between the two heatmaps use the Display: Mean Values and Fold Difference options.<BR /><BR />
You may use the buttons at the top left to control the size of the rows and columns.<BR /><BR />
The legend can be found next to the column and row size buttons.  This provides a reference for the range of the values displayed.<BR /><BR />
...
<H3>Heritability</H3>
Heritability is calculated from individual expression values in the panel of recombinant inbred rodents. The broad sense heritability is calculated from a 1-way ANOVA model comparing the within-strain variance to the between-strain variance. A higher heritability indicates more of the variance in expression is determined by genetic factors rather than non-genetic factors in this particular RI panel. This tab allows you to view the heritability of unambiguous probesets.  For Affymetrix exon arrays, probeset typically consist of 4 unique probes.  Prior to analysis, we eliminated (masked) individual probes if their sequence does not align uniquely to the genome or if the probe targets a region of the genome that harbored a known single nucleotide polymorphism (SNP) between the two parental strains of the RI panel.  Entire probesets were eliminated if less than three probes within the probeset remained after masking.   Probes that target a region with a known SNP may indicate dramatic differences in expression when expression levels are similar but hybridization efficiency differ.
<BR /><BR />
If a probeset of interest is missing try adjusting the filtering to allow additional probes (allow introns, opposite strand, make sure all other filters are unchecked, etc.) If it still doesn't appear it means that the probeset was masked and there is no heritability data because the probeset data would be inaccurate.

</div></div>

<div id="Help6Content" class="inpageHelpContent" title="<center>Help</center>"><div class="help-content">
<H3>Panel Expression</H3>
These are the normalized log transformed expression values.  This shows expression values for each probeset accross all strains in the panel.  Note: Because of the normalization you should not compare normalized values between different probesets, but you can compare them accross strains.  <BR /><BR />

There are three ways to view the data.  The default produces a serparate graph for each probeset.  Notice the range and size varies with each probeset.  The size varies directly with the range of values so you can quickly scan for more variable or consistenat probesets. <BR /><BR />

The next method if you view probesets grouped into one graph by tissue shows the variability by strain in a single graph.  This allows you to look for probsets that vary significantly between strains.   You should not compare expression between probesets along the X-axis as the normalization does not allow comparison of expression values between probesets.  <BR /><BR />

The other method will display probesets in a series accross strains.  Again it is important that you do not use this to compare expression values between probesets.  The best way to compare expression is to use the Exon Correlation Tab.<BR /><BR />

Masking: Probesets have been masked so, if a probeset of interest is missing try adjusting the filtering to allow additional probes(allow introns, opposite strand, make sure all other filters are unchecked, etc.) If it still doesn't appear it means that the probeset was masked and there is no heritability data because the probeset data would be inaccurate. Probesets have been masked because the sequence for the probeset doesn't match the strain of mice or rats and as a result the data from the probeset would be misleading.

</div></div>
<div id="Help7Content" class="inpageHelpContent" title="<center>Help</center>"><div class="help-content">
<H3>Exon Correlation</H3>
This tab allows you to compare probeset expression which should not be done directly in the expression tab.  This heatmap shows a selected transcript accross the top and draw exons that are represented by probesets along the top of the heatmap.  Exons excluded are color coded to match a legend at the bottom of the display as to why the exon is excluded from the heatmap.  Some exons may have multiple probesets representing them.<BR /><BR />
