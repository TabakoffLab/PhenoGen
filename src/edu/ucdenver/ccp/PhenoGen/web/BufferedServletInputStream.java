...
public class BufferedServletInputStream extends ServletInputStream {
  ...
  public BufferedServletInputStream(ServletInputStream in) {
	...
	log.debug("in constructor of BufferedServletInputStream");
  }
...
log.debug("in readLine of BSIS. len=" + len);
    int total = 0;
    if (len == 0) {
      return 0;
    }
...
log.debug("avail = " + avail);
    if (avail <= 0) {
log.debug("avail is <= 0, so filling");
      fill();
      avail = count - pos;
log.debug("now avail = " + avail);
      if (avail <= 0) {
log.debug("avail is <= 0, so returning -1");
        return -1;
      }
    }
...
log.debug("eol = " + eol);
    if (eol != -1) {
      copy = eol;
log.debug("eol != -1, so copy now = " + copy);
    }
log.debug("111");
