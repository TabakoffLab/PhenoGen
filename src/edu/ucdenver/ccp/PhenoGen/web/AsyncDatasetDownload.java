...
public class AsyncDatasetDownload implements Runnable{
	...
	public AsyncDatasetDownload(HttpServletRequest request, ArrayList<String> checkedList) {

		log = Logger.getRootLogger();
		log.debug("just instantiated AsyncDatasetDownload");
	...
	public void run() throws RuntimeException {
		log.debug("in AsyncDatasetDownload run");
		AsyncFileDownload thisFileDownload = new AsyncFileDownload(request, checkedList, selectedDataset.getDownloadsDir(), selectedDataset.getNameNoSpaces(),userLoggedIn, true);
	...
	public void sendEmail(User user, List<String> fileList) {
		log.debug("in AsyncDatasetDownload.sendEmail");
		int totalNumberOfFiles = fileList.size();
		Email myEmail = new Email();
		StringBuffer emailBody = new StringBuffer();
		emailBody.append(user.getFull_name() +", \n\n");
		emailBody.append("Your dataset is ready for download. ");
		myEmail.setSubject("Your PhenoGen Dataset Download");
