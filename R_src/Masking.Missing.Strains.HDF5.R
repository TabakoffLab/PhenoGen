...
Masking.Missing.Strains.HDF5<-function(InputDataFile,VersionPath,SampleFile, InputPhenoFile, OutputFileHDF,OutputFileSamples){
  ...
  H5Sclose(sid)
	...
	sid <- H5Screate_simple (dim(grouping), dim(grouping) )
	did <- H5Dcreate (gVersionOut, "Grouping", "H5T_STD_I32LE", sid)
	H5Dwrite(did,grouping)
	H5Dclose(did)
	H5Sclose(sid)
	sid <- H5Screate_simple (dim(Gnames), dim(Gnames) )
	did <- H5Dcreate (gVersionOut, "Probeset", "H5T_STD_I64LE", sid)
	H5Dwrite(did,Gnames)
	H5Dclose(did)
	H5Sclose(sid)
	sid <- H5Screate_simple (dim(Snames), dim(Snames) )
	did <- H5Dcreate (gVersionOut, "Samples", "H5T_STRING", sid)
	H5Dwrite(did,Snames)
	H5Dclose(did)
	H5Sclose(sid)
