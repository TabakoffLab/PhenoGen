...
#      }
 ...
 ## Check if Colv and dendrogram arguments are consistent
...
else
  {
    ...
    ## personal changes for INIA
    write.table(colnames(x),file="SampleTable.txt",col.names=FALSE,quote=FALSE)
    ...
    if (nc<15) axis(1, 1:nc, labels= labCol, las= 2, line= -0.5, tick= 0, cex.axis= cexCol)
    else axis(1, 1:nc, labels= FALSE, las= 2, line= -0.5, tick= 0, cex.axis= cexCol)

    if(!is.null(xlab)) mtext(xlab, side = 1, line = margins[1] - 1.25)
    if(length(labRow)>50){
      axis(4, iy, labels= FALSE, las= 2, line= -0.5, tick= 0, cex.axis= cexRow)
      }
    else axis(4, iy, labels= labRow, las= 2, line= -0.5, tick= 0, cex.axis= cexRow)
    write.table(rev(labRow),file="GeneTable.txt",sep="\t",col.names=FALSE,quote=FALSE)
    ...
    if(key)
      {
        ...
        if(scale=="row")
          mtext(side=1,"Row Z-Score", line=2,cex=0.8)
        else if(scale=="column")
