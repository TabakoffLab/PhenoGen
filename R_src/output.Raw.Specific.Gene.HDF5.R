...
####################################################
 ...
 output.Raw.Specific.Gene.HDF5 <- function(InputFile, VersionPath, SampleFile, GeneList, Platform, OutputFile, TypeOutput){
  ...
  colnames(Absdata)<-Snames
	...
	if(TypeOutput=="Individual"){
		...
		write.table(WithCalls,file=OutputFile,sep="\t",row.names=FALSE,quote=FALSE)
	}
	...
	if(TypeOutput=="Group"){
		...
		write.table(GroupSpecific,file=OutputFile,sep="\t",row.names=FALSE,quote=FALSE)
	} 
}
## END
