...
function drawGraph(drwNodes,drwLinks){
   ...
   if(typeof selectedNode !=='undefined'){
  	if(drwNodes.length<30){
    	linkDist=minDim/(5+edgeCutoff);
	}else{
		linkDist=minDim/(9+edgeCutoff);
	}
  }else{
  	if(drwNodes.length<30){
  		linkDist=minDim/8;
...
function addControls(){
  ...
  imageBar.append("span").attr("class","saveImage control").style("display","inline-block")
	...
	edgeCtl.append("span").attr("class","edgePlus control")
		 ...
		 .on("click",function(){
		   	edgeCutoff+=1;
		   	imageBar.select("#edgeCountLbl").html("Edge Distance Limit from selected node: "+edgeCutoff);
			redrawGraph(false);
		 })
		 .on("mouseover",function(){
	...
	edgeCtl.append("span").attr("id","edgeCountLbl").style({"padding-left":"10px","position":"relative","top":"-8px"}).html("Edge Distance Limit from selected node: "+edgeCutoff);
