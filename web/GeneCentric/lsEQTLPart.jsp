...
<% 
			...
			if(request.getParameter("hiddenGeneSymbol")!=null){
				...
				if(session.getAttribute("geneSymbol")!=null){
					geneSymbolinternal =(String)session.getAttribute("geneSymbol");
					geneCentricPath =(String)session.getAttribute("geneCentricPath");
					log.debug("Got geneCentricPath and geneSymbol from session variables "+ geneSymbolinternal + "  " + geneCentricPath);
				}else{
					geneSymbolinternal=(String)geneSymbol.get(selectedGene);
					geneCentricPath=gcPath;
					log.debug("Got geneCentricPath and geneSymbol from previously set page variables "+ geneSymbolinternal + "  " + geneCentricPath);
				}
			}
	...
	if (((action != null) && action.equals("Click to run Circos"))) {
		...
		if((!selectedChromosomeError)&&(!selectedTissueError)){
			...
			String perlEnvironmentVariables = (String)session.getAttribute("perlEnvVar");
 			...
 			String hostName=request.getServerName();
				...
				perlEnvironmentVariables += ":/bin:/usr/bin:/usr/bin/perl:/usr/local/circos-0.68/lib:/usr/local/circos-0.68/bin";
			}
			else{
				perlEnvironmentVariables += ":/usr/bin/perl5.10:/usr/local/circos-0.68/lib:/usr/local/circos-0.68/bin";
			}
...
<script>
	...
	<form	
		...
		<tr>	
				<td>
				<INPUT TYPE="submit" NAME="action" id="clickToRunCircos" Value="Click to run Circos" onClick="return displayWorkingCircos()">
				</td>
				<td>
...
<BR>
	...
	<div id="wait2" align="center"><img src="<%=imagesDir%>wait.gif" alt="Working..." text-align="center" >
