...
<% 
	AsyncGeneDataThread agdt=new AsyncGeneDataThread(session,gdt,chromosome,min,max,panel,myOrganism,rnaDatasetID,arrayTypeID,forwardPValueCutoff);
	agdt.start();
	while(!gdt.isPathReady()){
		try{
			agdt.join(1000);
		}catch(InterruptedException e){
		}
	}
	String tmpOutput=gdt.getOutputDir();
	int start=tmpOutput.lastIndexOf("/",tmpOutput.length()-2);
	String foldername=tmpOutput.substring(start,tmpOutput.length()-1);
