...
sub createCircosConfFile{
	...
	my ($confDirectory,$genericConfLocation,$genericConfLocation2,$karyotypeLocation,$organism,$chromosomeListRef,$oneToCreateLinks,$genomeVer) = @_;
	my @chromosomeList = @{$chromosomeListRef};
	my $numberOfChromosomes = scalar @chromosomeList;
	if($debugLevel >= 2){
	...
	if($organism eq 'Rn'){
		print CONFFILE 'karyotype   = '.$karyotypeLocation.'karyotype.rat.'.lc($genomeVer).'.txt'."\n";
	}
