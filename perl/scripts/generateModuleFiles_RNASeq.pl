...
foreach my $mod(@moduleList){
    ...
    for(my $row=0;$row<@adjTC;$row++){
        ...
        for(my $col=0;$col<$row;$col++){#only read first part up to the 1 since its symmetrical
            if(not($mod eq 'turquoise' or $mod eq 'blue' or $mod eq 'brown') or ($mod eq 'turquoise' and $columns[$col]>0.15) or (($mod eq 'blue' or $mod eq 'brown') and $columns[$col]>0.01)){
                ...
                #print("link:".$moduleHOH{LinkList}[$linkCount]{TC1}.":".$moduleHOH{LinkList}[$linkCount]{TC2}.":".$moduleHOH{LinkList}[$linkCount]{cor}."\n");
                $linkCount++;
            }
        }
    }
    close ADJ;
    my $query2="select unique wmi.TRANSCRIPT_CLUST_ID,rt.merge_gene_id,wmi.gene_id,wmi.module_id,wmc.rgb,wmc.hex 
    ...
    my $modRGB;
    my $modHex;
    $query_handle2->bind_columns(\$tc,\$altGeneID,\$geneid,\$modID,\$modRGB,\$modHex);
...
#            print OFILE "\n\t\t\t\t],\n";#end Transcript List
    ...
    my $eqtlAOHRef = readLocusSpecificPvaluesModule($mod,$org,$tissue,\@chrArr,$genomeVer,$dsn,$user,$passwd,"seq");
    ...
    my $dsNum=$wgcnaDataset;
    my $tmpPath=$tmpP."/circos/ds".$dsNum."/".$mod."/";
    #print "Circos Path:".$tmpPath."\n";
    my $cutoff=2;
    
    callCircosMod($mod,$cutoff,$org,$chrString,$tissue,$tmpPath,"1",$modRGB,$genomeVer,$dsn,$user, $passwd,"seq");
}