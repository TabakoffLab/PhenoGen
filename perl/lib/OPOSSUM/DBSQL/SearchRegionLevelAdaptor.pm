...
sub fetch_levels
{
    ...
    my $sql = qq{select search_region_level from search_region_levels
    		    order by search_region_level};
...
sub fetch_search_region_levels
{
    ...
    my $sql = qq{select search_region_level, upstream_bp, downstream_bp
		    from search_region_levels order by search_region_level};
...
sub fetch_by_level
{
    ...
    my $sql = qq{
		...
		where search_region_level = $level};
	...
	$crl = OPOSSUM::SearchRegionLevel->new(
				    -level		=> $level,
				    -upstream_bp	=> $row[0],
				    -downstream_bp	=> $row[1]);
    }
