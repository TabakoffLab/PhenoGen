...
$cra = $db_adaptor->get_ConservedTFBSAdaptor();

=head1 CHANGE HISTORY

=head1 AUTHOR

 Modified by SHS on Dec 27, 2006
   Added fetch_gene_pair_ids to replace fetch_promoter_pair_ids as
     conserved_tfbss table no longer stores promoter_pair_ids.
   Modified fetch_tf_ids to access conserved_tfbss table.
   Added fetch_by_gene_pair method.

 Modified by SHS on January 10, 2007
   Added _combine_search_regions_pairs method and modified
     fetch_by_gene_pair to use it.
   The fetch_by_gene_pair_method returns the promoter start/end
     positions containing the TFBS but does not return the TSSs
     as there may be multiple TSSs within each promoter.

 Modified by DJA on January 29, 2007
   Added _define_search_regions method and modified fetch_tfbs_count
     and fetch_tfbs_counts methods to call it.
   Updated fetch_tfbs_count and fetch_tfbs_counts methods to return count(s)
     on a per search region basis and combine results instead of having
     a big where clause with all the search regions (more efficient???).
...
sub fetch_by_gene_pair
{
    ...
    $table .= "_" . $self->{-extended_table_name}
    			if $self->{-extended_table_name};
    my $sql = qq{
	        select tf_id, start1, end1, strand1, seq1,
	            rel_score1, start2, end2, strand2, seq2,
		    rel_score2, conservation_level, conservation
		from $table 
		where gene_pair_id = $gpid
		    and conservation_level >= $clevel
		    and tf_id = $tfid
    ...
    while (my @row = $sth->fetchrow_array) {
	push @tf_sites, OPOSSUM::ConservedTFBS->new(
				    -id                 => $row[0],
				    -start1             => $row[1],
				    -end1               => $row[2],
				    -strand1            => $row[3],
...
sub fetch_by_gene_pair_id
{
    ...
    $table .= "_" . $self->{-extended_table_name}
    			if $self->{-extended_table_name};
    my $sql = qq{
		select tf_id, start1, end1, strand1, seq1, score1, start2,
			end2, strand2, seq2, score2, level, conservation
		from $table
