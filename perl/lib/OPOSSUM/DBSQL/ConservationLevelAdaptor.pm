...
sub fetch_levels
{
    ...
    my $sql = qq{select conservation_level from conservation_levels
    		    order by conservation_level};
...
sub fetch_conservation_levels
{
    ...
    my $sql = qq{select conservation_level, top_percentile, min_percentage
		    from conservation_levels order by conservation_level};
	...
	$levels->add_conservation_level(
		    OPOSSUM::ConservationLevel->new(
					    -level		=> $row[0],
					    -top_percentile	=> $row[1],
					    -min_percentage	=> $row[2]));
    }
    $sth->finish;
...
sub fetch_by_level
{
    ...
    my $sql = qq{
    		select top_percentile, min_percentage
		from conservation_levels
		where conservation_level = $level};
...
sub store
{
    ...
    my $sql = qq{insert into conservation_levels (
    			conservation_level, top_percentile, min_percentage)
		values (?,?,?)};
