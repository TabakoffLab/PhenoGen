...
sub fetch_id_types
{
    ...
    my $sql = qq{select id_type from external_gene_id_types};
...
sub fetch_names
{
    ...
    my $sql = qq{select name from external_gene_id_types};

    my $sth = $self->prepare($sql);
    if (!$sth) {
	carp "error fetching ID type names\n" . $self->errstr;
	return;
    }

    if (!$sth->execute) {
	carp "error fetching ID type namess\n" . $self->errstr;
	return;
    }
...
sub fetch_by_id_type
{
    ...
    my $sql = qq{select id_type, name from external_gene_id_types
    		where id_type = $id};

    my $sth = $self->prepare($sql);
    if (!$sth) {
	carp "error fetching external gene ID type\n" . $self->errstr;
	return;
    }
...
=head2 fetch
 ...
 Title    : fetch
 Usage    : $xgit = $xgita->fetch($where_clause);
