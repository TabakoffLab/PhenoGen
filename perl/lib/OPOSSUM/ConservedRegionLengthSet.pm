...
=head2 total_conserved_region_length
 ...
 Usage    : $length = $crls->total_conserved_region_length($id);
 Function : Return the total length of the conserved region length
 	    with the given ID
 Returns  : An integer
...
sub add_conserved_region_length
{
    ...
    $self->{_crl_set}->{$crl->gene_pair_id} = $crl;
}
