...
sub write_counts
{
	...
	carp "no counts provided or counts is not an OPOSSUM::Analysis::Counts"
		. " object";
	return;
    }
...
sub _read_detail_counts
{
	...
	if ($line =~ /^>TFBS/) {
		...
		if (scalar @counts != $num_tfbs) {
		    carp "number of counts read does not match number of TFBSs"
		    		. " for gene number " . $genes_read + 1
				. " ID " . $gene_ids[$genes_read];
		    return;
		}
...
sub _write_fisher_counts
{
    ...
    my $tf_ids = $counts->get_all_tf_ids();
    if (!$tf_ids) {
