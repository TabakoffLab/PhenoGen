...
=head1 SYNOPSIS
 ...
 $results = $zscore->calculate_zscore(
					    $background_counts,
					    $target_counts);
...
sub calculate_Zscore
{
    ...
    my $C;
    if ($bg_cr_len) {
	$C = $t_cr_len / $bg_cr_len;
    } else {
        carp "Ratio of conserved region lengths is undefined";
	return;
    }
    ...
    my $t_tf_ids = $t_counts->get_all_tf_ids;
	...
	# PWM width
    	my $width = $t_counts->tfbs_width($tf_id);
