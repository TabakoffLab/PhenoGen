...
sub tf_sites
{
	...
	if ($sort_by eq 'name' || $sort_by eq 'tfbs_name') {
	    return sort {$a->tfbs_name cmp $b->tfbs_name
			    || $a->start1 <=> $b->start1
			    || $a->strand1 cmp $b->strand1
			} @{$self->{_site_array}};
	} elsif ($sort_by eq 'score' || $sort_by eq 'score1') {
	    return sort {$a->score1 <=> $b->score1
			    || $a->start1 <=> $b->start1
			    || $a->strand1 cmp $b->strand1
			} @{$self->{_site_array}};
	} elsif ($sort_by eq 'score2') {
	    return sort {$a->score2 <=> $b->score2
			    || $a->start2 <=> $b->start2
			    || $a->strand2 cmp $b->strand2
	    		} @{$self->{_site_array}};
	} elsif ($sort_by eq 'start' || $sort_by eq 'start1') {
	    return sort {$a->start1 <=> $b->start1
			    || $a->tfbs_name cmp $b->tfbs_name
			    || $a->strand1 cmp $b->strand1
	    		} @{$self->{_site_array}};
	} elsif ($sort_by eq 'start2') {
	    return sort {$a->start2 <=> $b->start2
			    || $a->tfbs_name cmp $b->tfbs_name
			    || $a->strand2 cmp $b->strand2
	    		} @{$self->{_site_array}};
	} elsif ($sort_by eq 'end' || $sort_by eq 'end1') {
	    return sort {$a->end1 <=> $b->end1
			    || $a->tfbs_name cmp $b->tfbs_name
			    || $a->strand1 cmp $b->strand1
	    		} @{$self->{_site_array}};
	} elsif ($sort_by eq 'end2') {
	    return sort {$a->end2 <=> $b->end2
			    || $a->tfbs_name cmp $b->tfbs_name
			    || $a->strand2 cmp $b->strand2
	    		} @{$self->{_site_array}};
